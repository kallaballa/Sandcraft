...
void Particles::InertParticleLogic(int x, int y, ParticleType type) {
	...
	case IRONWALL:
		...
		right = (x - 1) + (y * config_.width_);
		if (utils::rand() % 200 == 0
				&& (vs_[above] == RUST || vs_[left] == RUST || vs_[right] == RUST))
...
inline void Particles::MoveParticle(int x, int y, ParticleType type) {
	...
	case MOVEDACID:
		...
		if (vs_[index] != WALL && vs_[index] != IRONWALL && vs_[index] != WATER
				&& vs_[index] != MOVEDWATER && vs_[index] != ACID
				&& vs_[index] != MOVEDACID)
			vs_[index] = NOTHING;
		break;
		break;
...
void Particles::DrawParticles(int xpos, int ypos, int radius, ParticleType type) {
