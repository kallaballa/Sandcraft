...
EM_BOOL web_socket_open(int eventType, const EmscriptenWebSocketOpenEvent *e, void *userData)
{
...
EM_BOOL web_socket_close(int eventType, const EmscriptenWebSocketCloseEvent *e, void *userData)
{
	printf("close(eventType=%d, wasClean=%d, code=%d, reason=%s, userData=%ld)\n", eventType, e->wasClean, e->code, e->reason, (long)userData);
	return 0;
}

EM_BOOL web_socket_error(int eventType, const EmscriptenWebSocketErrorEvent *e, void *userData)
{
...
bool is_init = false;
EM_BOOL web_socket_message(int eventType, const EmscriptenWebSocketMessageEvent *e, void *userData)
{
	using namespace scserver;
	...
	if (e->isText) {
		...
		if(startsWith(str, "Error:")) {
			...
			assert(!lines.empty());
			if (lines.size() == 1) {
				WebsocketClient::rtc_->init(
						[=](Description desc) {
							if(WebsocketClient::is_host_)
							return;
							WebsocketClient::is_host_ = true;
				...
				if(!is_init) {
					WebsocketClient::rtc_->dc_ = WebsocketClient::rtc_->pc_->createDataChannel("test");
				}
				is_init = true;
			} else {
				...
				for(
					...
					if (participants < max) {
						WebsocketClient::rtc_->init(
								[=](Description desc) {
									WebsocketClient::is_host_ = false;
				...
				WebsocketClient::rtc_->answer(Description(selSDP, "answer"));
			}
			...
			WebsocketClient::rtc_->offer(Description(sdp, "offer"));
			...
			WebsocketClient::rtc_->pc_->addRemoteCandidate(Candidate(cand, mid));
...
bool WebsocketClient::is_host_ = false;
WebRTC* WebsocketClient::rtc_ = nullptr;

WebsocketClient::WebsocketClient(int port, WebRTC* rtc) {
	rtc_ = rtc;
	...
	attr.url = (std::string("ws://localhost:") + std::to_string(port) + "/sandcraft").c_str();
	...
	emscripten_websocket_set_onopen_callback(socket, (void*)42, web_socket_open);
	emscripten_websocket_set_onclose_callback(socket, (void*)43, web_socket_close);
	emscripten_websocket_set_onerror_callback(socket, (void*)44, web_socket_error);
	emscripten_websocket_set_onmessage_callback(socket, (void*)45, web_socket_message);
}
